...
import { Router } from 'express';
// import { readDataFile, writeDataFile } from '../utils/fileUtils'; // Deprecated
import { v4 as uuidv4 } from 'uuid';
import { GoogleGenerativeAI } from '@google/generative-ai';
...
// =========================================================================
// CRUD Routes (Migrated to Firestore via ProjectService)
// =========================================================================
...
router.get('/:projectId/scripts', async (req, res) => {
  try {
    const { projectId } = req.params;
    // @ts-ignore
    const userId = String(req.user?.uid || req.headers['x-user-id'] || 'user-id');
    // Fallback to 'user-id' only if auth missing, but try header too.

    const scripts = await projectService.getScripts(projectId, userId);
    res.json(scripts || []);
  } catch (error) {
    console.error('Error loading scripts:', error);
    res.json([]);
...
router.post('/:projectId/scripts', async (req, res) => {
  try {
    ...
    const newScript = await projectService.createScript(projectId, scriptData, userId);
    console.log(`[Script Route] 📝 Created Script: ${newScript.name || 'Untitled'} [ID: ${newScript.id}]`);
    res.status(201).json(newScript);
  } catch (error) {
    console.error('Error creating script:', error);
    res.status(500).json({ error: 'Failed to create script' });
...
router.get('/:projectId/scripts/:scriptId', async (req, res) => {
  try {
    ...
    // @ts-ignore
    const userId = String(req.user?.uid || req.headers['x-user-id'] || 'user-id');
    const scripts = await projectService.getScripts(projectId, userId); // Optimization: Add single getScript to service?
    // ProjectService had getScripts (collection scan). 
    // Let's rely on finding it in the array for now or improve service later.
...
router.put('/:projectId/scripts/:scriptId', async (req, res) => {
  try {
    ...
    // @ts-ignore
    const userId = String(req.user?.uid || req.headers['x-user-id'] || 'user-id');
    const updatedScript = await projectService.updateScript(projectId, scriptId, updates, userId);
    console.log(`[Script Route] ✏️ Updated Script: ${updatedScript.name || scriptId} [ID: ${scriptId}]`);
...
router.delete('/:projectId/scripts/:scriptId', async (req, res) => {
  try {
    ...
    // @ts-ignore
    const userId = String(req.user?.uid || req.headers['x-user-id'] || 'user-id');
    await projectService.deleteScript(projectId, scriptId, userId);
    res.json({ message: 'Script deleted successfully' });
  } catch (error) {
    console.error('Error deleting script:', error);
    res.status(500).json({ error: 'Failed to delete script' });
...
router.post('/:projectId/scripts/:scriptId/run', async (req, res) => {
  try {
    ...
    // @ts-ignore
    const userId = String(req.user?.uid || req.headers['x-user-id'] || 'user-id');
    // const data = await readDataFile(`project-${projectId}-data.json`); // Removed legacy file read
    // const scripts = data.scripts || [];

    const script = await projectService.getScript(projectId, scriptId, userId);
    if (!script) {
      return res.status(404).json({ error: 'Script not found' });
    }

    // REAL Execution Integration
    // This reconnects the Studio directly to the TestRunnerService
    const { testRunnerService } = await import('../../services/execution/TestRunnerService');
    const { testRunService } = await import('../../services/persistence/TestRunService');

    // 1. Execute the Test
    console.log(`[Script] Executing script ${scriptId} via Real Engine...`);
    const executionResult = await testRunnerService.executeTest(
      scriptId,
      projectId,
      'manual',
      userId
    ...
    // 2. Fetch the Logs (since executeTest returns runId)
    // We need to wait slightly for logs to be flushed or just fetch what is there.
    // Since executeTest awaits completion, logs should be ready.
    const runDetails = await testRunService.getRunDetails(projectId, executionResult.runId);

    // 3. Map status for Frontend (passed -> pass)
    const status = executionResult.status === 'passed' ? 'pass' : 'fail';

    // 4. Format Logs
    let logsString = '';
    if (runDetails && runDetails.logs) {
      if (Array.isArray(runDetails.logs)) {
        // Handle both string[] and object[] logs (if any)
        logsString = runDetails.logs.map((l: any) => {
          if (typeof l === 'string') return l;
          return `[${l.status?.toUpperCase() || 'INFO'}] ${l.action}: ${l.message}`;
        }).join('\n');
      } else {
        logsString = String(runDetails.logs);
      }
    } else {
      logsString = executionResult.error || "Test completed (No logs returned)";
    }
